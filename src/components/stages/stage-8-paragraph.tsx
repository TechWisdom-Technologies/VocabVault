...
export default function Stage8Paragraph({ word, onComplete }: Stage8Props) {
  ...
  return (
    <div className="flex-1 w-full h-full flex flex-col overflow-hidden bg-[#0a0a0b] relative pt-20">
      <div className="flex-1 w-full h-full flex flex-col overflow-hidden relative">
        ...
        <div className="flex-[1] bg-violet-600/10 relative flex flex-col p-6 sm:p-12 min-h-0">
          ...
          <div className="flex justify-between items-start z-10 mb-6 shrink-0">
            <div className="flex items-center gap-3">
              ...
              <span className="text-[10px] font-black uppercase tracking-[0.3em] text-white/40">Contextual Reference</span>
            </div>

            <div className={`px-4 py-1.5 rounded-full border border-white/10 flex items-center gap-2 ${timeLeft < 30 ? "bg-rose-500/20 border-rose-500/40" : "bg-white/5"}`}>
              <Clock className={`w-3.5 h-3.5 ${timeLeft < 30 ? "text-rose-400 animate-pulse" : "text-violet-400"}`} />
              <span className={`text-sm font-mono font-black ${timeLeft < 30 ? "text-rose-400 animate-pulse" : "text-white"}`}>{timeFormatted}</span>
            </div>
          </div>

          <div className="max-w-4xl mx-auto w-full flex-1 overflow-y-auto custom-scrollbar relative z-10 pt-4">
            <p className="text-lg sm:text-2xl lg:text-3xl text-white font-medium leading-relaxed sm:leading-loose">
              {paragraph}
            </p>
          </div>
        </div>
        ...
        <div className="h-px w-full bg-linear-to-r from-transparent via-white/10 to-transparent relative z-20">
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex items-center gap-2 px-6 py-2 rounded-full bg-black border border-white/10 shadow-2xl">
            <Sparkles className="w-3.5 h-3.5 text-violet-400" />
            <span className="text-[10px] font-black text-white/60 uppercase tracking-[0.3em]">Validation Phase</span>
          </div>
        </div>

        {/* Zone B: The Analysis Station (Bottom 50%) */}
        <div className="flex-[1] bg-black p-6 sm:p-12 flex flex-col min-h-0">
            ...
            {[
              ...
              <div key={idx} className="flex flex-col items-center gap-6 group">
                <div className="flex flex-col items-center gap-2">
                  <div className={`p-3 rounded-2xl bg-white/5 border border-white/10 text-white/20 group-hover:text-white group-hover:border-violet-500/50 transition-all shadow-xl`}>
                    {item.icon}
                  </div>
                  <span className="text-[11px] font-black uppercase tracking-widest text-white/30 group-hover:text-white transition-colors">{item.label}</span>
                </div>
                <Input
                  type="number"
                  value={item.value}
                  onChange={(e) => item.setter(e.target.value)}
                  className="text-center text-4xl sm:text-5xl font-black bg-transparent border-0 focus-visible:ring-0 placeholder:text-white/5 h-12 sm:h-16 p-0"
                  placeholder="0"
                />
              </div>
            ))}
          ...
          {/* Action Controls */}
          <div className="mt-auto flex flex-col sm:flex-row items-center justify-between gap-6 pt-8 border-t border-white/5 w-full max-w-6xl mx-auto shrink-0">
            <Button variant="ghost" onClick={handleRepeat} className="text-white/20 hover:text-white/80 hover:bg-white/5 text-[10px] font-black uppercase tracking-widest gap-2">
